<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return mixed
    */
    /*public function queryGen($query, $backtick_protect = TRUE)
    {
      foreach($this->explode(',', $columns) as $val)
      {
        $column = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
    ...
    * @return mixed
    */
    public function order_by($key, $val)
    {
      //$this->group_by[] = $val;
    ...
    */
    private function get_filtering()
    {
      ...
      if (!empty($mColArray)) {
	     $countMColArray = count($mColArray); 
      } else {
       $countMColArray = 0; 
      }
	  ...
	  $colArrayNew = $countMColArray - 1;
	  //print_r($colArrayNew);exit;
      if($sSearch != '')
        for($i = 0; $i < $colArrayNew; $i++)
