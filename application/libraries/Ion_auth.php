...
class Ion_auth
{
	 ...
	 * @author Mathew
	 */
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array(), $id_user_aplikasi)
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $group_ids, $id_user_aplikasi);
	...
	public function in_group($check_group, $id = FALSE, $check_all = FALSE)
	{
		...
		if (isset($this->_cache_user_in_group[$id])) {
			...
			foreach ($users_groups as $group) {
				$groups_array[$group->id] = $group->type;
			}
